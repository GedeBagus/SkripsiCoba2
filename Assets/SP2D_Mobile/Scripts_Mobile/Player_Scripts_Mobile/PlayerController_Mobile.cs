...
namespace Bitboys.SuperPlaftormer2D {
	...
	void Awake () {
		...
		void Update (){ // In the Update function we include all the actions that we want that work in every frame.
		...
		if (vortexIntro == true) {
			...
			if (isInGame == true) {
				...
				if (weapon1.gotWeapon1 || weapon2.gotWeapon2 || weapon3.gotWeapon3 || weapon4.gotWeapon4) {
					...
					if (controllerActive == true && CrossPlatformInputManager.GetButtonDown ("SWR") && !touchingLeftWall && !touchingRightWall && !inWaterZone) { // Play a sound if we press the switch between weapons button or key.
	...
	// if we touch the left or right wall the gravity scale changes to avoid the player fall down so quickly. Also we apply some horizontal force to get the sticky feeling.
		if (touchingLeftWall || touchingRightWall) {
			...
			//  Left to right.
			if (controllerActive == true &&  touchingLeftWall && Input.GetKey (KeyCode.RightArrow) && !isGrounded|| touchingLeftWall && CrossPlatformInputManager.GetAxis ("Horizontal_Mobile") > 0.5f && !isGrounded) {
